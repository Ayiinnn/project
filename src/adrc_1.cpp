...
void ADRC_Init(Fhan_Data* fhan_Input, float r, float h, float z, float p, float d)
{
	...
	fhan_Input->beta_0 = 10.0f;//控制律积分环节，不启用------现启用
	fhan_Input->beta_1 = p;
	fhan_Input->beta_2 = d;
	...
	fhan_Input->zeta = z;
	fhan_Input->b = 1.0f;//ESO扰动补偿，暂时不启用-------现启用
	fhan_Input->alpha0 = 0.2f;//控制律积分环节非线性参数，不启用(原-0.2f)
	fhan_Input->e0 = 0;
...
void Nolinear_Conbination_ADRC(Fhan_Data* fhan_Input)
{
	...
	fhan_Input->u0 = fhan_Input->beta_1 * Fal_ADRC(fhan_Input->e1, fhan_Input->alpha1, fhan_Input->zeta)
		+ fhan_Input->beta_0 * Fal_ADRC(fhan_Input->e0, fhan_Input->alpha0, fhan_Input->zeta)     //为何不启用？---------现启用
...
void ADRC_Control(Fhan_Data* fhan_Input, float expect_ADRC, float feedback_ADRC)
{
	...
	/* 加入简单积分项（用于测试）
	float max_inte=expect_ADRC;
	float diff_rate=abs(feedback_ADRC-expect_ADRC)/expect_ADRC;
	std::cout<<diff_rate<<std::endl;
	if (0.001<diff_rate && diff_rate <0.04 && fhan_Input->inte <= max_inte)
	{
        fhan_Input->inte+=expect_ADRC-feedback_ADRC;
	}
	fhan_Input->u+=0.10*fhan_Input->inte;
	*/
}
